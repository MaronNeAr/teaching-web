    ...
    <div class="emoji">
        ...
        <transition name="showbox">
            <div class="emojiBox" v-show="showEmoji">
                <li v-for="(item, i) in emojis" :key="i">
                    <img :src="'static/emoji/'+item.file" :data="item.code" @click="content +=item.code">
                </li>
            </div>
        </transition>
        <el-select style="float:right" placeholder="选择用户" v-model="user">
            <el-option v-for="item in users" :label="item.name" :value="item.name" :key="item.uid" @click="select(item.uid)"></el-option>
        </el-select>
    </div>
        ...
        function send() {
            if (this.content.length <= 1) {
                ...
                if (this.channelname === '机器人') {
                    ...
                    params.append("cid", this.$store.state.selectId);
                    params.append("uid", this.$store.state.userId);
                    axios.post("/send", params).then(response => {
